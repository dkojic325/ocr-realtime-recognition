...
def extract_roi():
    ...
    try:
        ...
        # print_screen = np.array(ImageGrab.grab(bbox))
        print_screen = np.array(ImageGrab.grab(bbox=win32gui.GetWindowRect(window)))
        print_screen = cv2.cvtColor(print_screen, cv2.COLOR_BGR2RGB)
        HEIGHT, WIDTH = print_screen.shape[:2]

        # convert image to HSV color space
        hsv_image = cv2.cvtColor(print_screen, cv2.COLOR_RGB2HSV)
        # make the color filter mask
        mask = cv2.inRange(hsv_image, (0, 0, 25), (255, 255, 45))
        ...
        for contour in contours:
            (x,y,w,h) = cv2.boundingRect(contour)
            if (h > HEIGHT * 0.75):
                large_contour.append((x,y,w,h))

        left = large_contour[0][0] + large_contour[0][2]
        top = large_contour[0][1]            
        right = large_contour[1][0]
        bottom = large_contour[0][1] + large_contour[0][3]

        # cv2.rectangle(print_screen, (left, top), (right, bottom), (0, 0, 255), 2)
        # cv2.imshow('screen', print_screen)
        # cv2.waitKey(500)

        roi = print_screen[top:bottom, left:right]
        return True, roi
...
if __name__ == '__main__':
    ...
    # # input the image
    # input_image = cv2.imread('./image/2.png')
    # analysis_image(input_image)

    winlist = []

    while True:
        ret, image = extract_roi()
        ...
        if image is not None:
            try:
                image = cv2.resize(image, (685, 945))
